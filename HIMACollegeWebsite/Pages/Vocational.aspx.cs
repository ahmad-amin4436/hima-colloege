...
namespace HIMACollegeWebsite
{
    public partial class Vocational : System.Web.UI.Page
    {
        ...
        private void BindSections()
        {
            try
            {
                string query = "SELECT Category FROM H_Programs WHERE Category LIKE 'Vocational%' GROUP BY Category ORDER BY Category ASC";
                ...
                if (dtCategories != null && dtCategories.Rows.Count > 0)
                {
                    ...
                    rpVocDepartments.DataSource = categories;
                    rpVocDepartments.DataBind();
                }
            }
            catch (Exception)
            {
        ...
        protected void rpVocDepartments_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                string categoryName = (string)e.Item.DataItem;
                Repeater rpInner = (Repeater)e.Item.FindControl("rpVocPrograms");
        ...
        protected void rpVocPrograms_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
